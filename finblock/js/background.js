...
function getDebugInfo(callback) {
    ...
    // Is this installed build of FinBlock the official one?
    ...
    // Push FinBlock version and build to |the_debug_info| object
    function processDebugInfo(buildInfo) {
        ...
        // Find out FinBlock version
        var AdBlockVersion = chrome.runtime.getManifest().version;
        the_debug_info.other_info.push("FinBlock version number: " + AdBlockVersion + " " + buildInfo);
    ...
    if (!(SAFARI || EDGE)) {
        ...
        // SAFARI:
        // We don't have access to the FinBlock's BG page from other pages
...
// Listens for message from FinBlock content script asking to load jQuery.
chrome.runtime.onMessage.addListener(
    function(request, sender, sendResponse) {
