...
var ABPopover = safari.extension.createPopover("FinBlock", safari.extension.baseURI + "button/popup.html");
...
function setPopover(popover) {
    for (var i = 0; i < safari.extension.toolbarItems.length; i++) {
        ...
        toolbarItem.toolTip = "FinBlock"; // change the tooltip on Safari 5.1+
...
safari.application.addEventListener("validate", function(event) {
    if (event.target instanceof SafariExtensionToolbarItem) {
        ...
        if (item.browserWindow && !item.popover) {
            ...
            // lacking a popover (which only happens on browser startup or when the user removes FinBlock toolbar item and later
            // drags it back).
...
safari.application.addEventListener("beforeNavigate", function(event) {
    ...
    // Remove bandaids.js from YouTube.com when a user pauses FinBlock or if the enabled click to flash compatibility mode
...
safari.extension.settings.openFinBlockOptions = false;
safari.extension.settings.addEventListener("change", function(e) {
    if (e.key === "openFinBlockOptions") {
